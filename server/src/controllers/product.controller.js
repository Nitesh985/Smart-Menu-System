import { Product } from "../models/food.models.js";
...
const searchProduct = asyncHandler(async (req, res)=>{
  ...
  const productFromName = await Product.find({name:query})
  const categoryFromName = await Category.findOne({name:query})
  

  const categoryId = categoryFromName?._id


  const productFromCategory = await Product.find({categoryId})


  if (!productFromName & !productFromCategory){
    throw new ApiError(404, "The product was not found")
  }
  ...
  return res.status(202).json({
    status:202,
    data,
    message:"The products are found"
  })
...
export { getAllProducts, getProduct, getProductsByCategory, uploadProduct, getReviews, getReviewsByStars, searchProduct};
