...
const SelectPayment = ({paymentOptions, setPaymentType}:SelectPaymentProps) => {
  return (
    <div className="w-[300px] p-2 aspect-square rounded-lg shadow flex flex-col items-center justify-center gap-2 bg-slate-50">
      ...
      <p className="text-[10px] self-start text-wrap text-gray-500 pb-1">
        ...
        <span className="font-bold underline">`Please don't use real credentials`</span>
      </p>
      {paymentOptions.map((item)=><label className="inline-flex justify-between w-full items-center z-10 rounded-lg p-2 border border-transparent has-[:checked]:border-indigo-500 has-[:checked]:text-indigo-900 has-[:checked]:bg-indigo-50 has-[:checked]:font-bold hover:bg-slate-200 transition-all cursor-pointer has-[:checked]:transition-all has-[:checked]:duration-500 duration-500 relative [&_p]:has-[:checked]:translate-y-0 [&_p]:has-[:checked]:transition-transform [&_p]:has-[:checked]:duration-500 [&_p]:has-[:checked]:opacity-100 overflow-hidden">
        ...
        <input className="checked:text-indigo-500 checked:ring-0 checked:ring-current focus:ring-0 focus:ring-current" defaultValue="google" name="payment" type="radio" onClick={()=>setPaymentType(item.label)} />
      </label>)}
    </div>
  );
}
